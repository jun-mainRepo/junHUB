...
class itg_admin {
  ...
  /*  public function get_nicename() {
        ...
        global $db;
        $info = $db->get_row("SELECT `username` FROM `user_info` WHERE `username` = '" . $db->escape($username) . "'");
        if(is_object($info))
  ...
  /*  public function get_email() {
    ...
    public function _login_action() {
        ...
        if($this->_check_db($username, $password)) {
           ...
           mysql_query("insert into visitors_table values('','','','','','$username','Login',now())")or die(mysql_error()) ;


        $us=$_POST["username"];
        $pass=$_POST["password"];
      $_SESSION['g_name']=$us;
       $_SESSION['pass_form']=$pass;


   $query=mysql_query("SELECT * FROM `user_info` WHERE `username` = '".$_SESSION['g_name']."'") or die(mysql_error()) ;
   ...
   $division=$result['division'];
      ...
      $_SESSION['division']=$division;
        ...
        /* $query=mysql_query("SELECT 'fname' FROM `user_info` WHERE `username` = '" .$_SESSION['admin_login']."'") or die(mysql_error()) ;
           $result=mysql_fetch_array($query);
           $fname=$result['fname'];

           $_COOKIE['g_name']=$fname;      */
    ...
    private function _check_db($username, $password) {
        global $db;
        $user_row = $db->get_row("SELECT * FROM `user_info` WHERE `username`='" . $db->escape($username) . "'");
    ...
    /* private function _check_db1($username, $password) {
        global $db;
        $user_row1 = $db->get_row("SELECT 'fname' FROM `user_info` WHERE `username`='" . $db->escape($username) . "'");


        //general return
        if(is_object($user_row1) && md5($user_row->password) == $password)
            return true;
        else
            return false;
    }   */
